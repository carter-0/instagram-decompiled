package X;

import java.io.IOException;

public final class F6Z {
    public static DwH parseFromJson(16F r3) {
        0qQ.A0B(r3, 0);
        try {
            1XQ r0 = new 1XQ();
            if (r3.A11() != 16L.A0D) {
                r3.A0z();
                return null;
            }
            while (r3.A1J() != 16L.A09) {
                String A17 = AnonymousClass7TE.A17(r3);
                if ("valid".equals(A17)) {
                    r0.A08 = r3.A0d();
                } else if ("available".equals(A17)) {
                    r0.A06 = r3.A0d();
                } else if ("server_corrected_email".equals(A17)) {
                    r0.A01 = AnonymousClass7TG.A0l(r3);
                } else if ("valid_nonce".equals(A17)) {
                    r0.A03 = AnonymousClass7TG.A0l(r3);
                } else if ("gdpr_required".equals(A17)) {
                    r0.A07 = r3.A0d();
                } else if ("gdpr_s".equals(A17)) {
                    r0.A00 = AnonymousClass7TG.A0l(r3);
                } else if ("tos_version".equals(A17)) {
                    r0.A02 = AnonymousClass7TG.A0l(r3);
                } else if ("suggested_email".equals(A17)) {
                    C41846B3n.A1A(r3);
                } else if ("age_required".equals(A17)) {
                    r0.A04 = r3.A0d();
                } else if ("allow_shared_email_registration".equals(A17)) {
                    r0.A05 = r3.A0d();
                } else {
                    1XY.A01(r3, r0, A17);
                }
                r3.A0z();
            }
            return r0;
        } catch (IOException e) {
            throw e;
        } catch (Exception e2) {
            throw AnonymousClass7TE.A0j(e2);
        }
    }
}
