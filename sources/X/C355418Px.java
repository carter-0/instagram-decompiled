package X;

import android.graphics.Bitmap;
import android.graphics.Canvas;
import android.graphics.Paint;
import android.graphics.PorterDuff;
import android.graphics.PorterDuffXfermode;
import android.graphics.RectF;

/* renamed from: X.8Px  reason: invalid class name and case insensitive filesystem */
public final class C355418Px extends C355378Pt {
    public final void draw(Canvas canvas) {
        Canvas canvas2 = canvas;
        0qQ.A0B(canvas2, 0);
        int width = getBounds().width();
        int height = getBounds().height();
        float width2 = ((float) getBounds().width()) * this.A00;
        float f = (float) this.A0D;
        float f2 = width2 + f;
        float width3 = (((float) getBounds().width()) * this.A01) - f;
        int i = this.A04;
        RectF rectF = new RectF();
        Bitmap createBitmap = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);
        0qQ.A07(createBitmap);
        Canvas canvas3 = new Canvas(createBitmap);
        Paint paint = new Paint(1);
        paint.setColor(i);
        paint.setStyle(Paint.Style.FILL);
        float f3 = (float) height;
        canvas3.drawRect(0.0f, 0.0f, (float) width, f3, paint);
        paint.setColor(0);
        paint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.DST_ATOP));
        rectF.set(f2, 0.0f, width3, f3);
        float f4 = (float) this.A0C;
        canvas3.drawRoundRect(rectF, f4, f4, paint);
        canvas2.drawBitmap(createBitmap, 0.0f, 0.0f, (Paint) null);
        super.draw(canvas2);
    }
}
