package X;

import android.view.View;

/* renamed from: X.FeI  reason: case insensitive filesystem */
public final class C50547FeI implements G7C {
    public final Dd4 A00;

    public C50547FeI(Dd4 dd4) {
        this.A00 = dd4;
    }

    public final boolean AFM(C48087EVf eVf, C283155Gi r4, int i) {
        AnonymousClass7TG.A1N(r4, eVf);
        if (eVf == C48087EVf.END_IMAGE && r4.A07() == null && this.A00.A0T(r4)) {
            return true;
        }
        return false;
    }

    public final void AJS(View view, JV7 jv7, C48087EVf eVf, C283155Gi r6, int i) {
        AnonymousClass7TG.A1N(r6, eVf);
        this.A00.DPx(C47179Dru.A00(jv7, eVf, i), r6, i);
    }
}
