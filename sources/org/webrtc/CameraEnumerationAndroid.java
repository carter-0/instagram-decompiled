package org.webrtc;

import X.AnonymousClass7TE;
import X.AnonymousClass7TF;
import X.C13988Tno;
import X.Pxf;
import android.graphics.ImageFormat;
import com.facebook.cameracore.instagram.ardelivery.networkconsentmanager.IgNetworkConsentStorage;
import java.util.ArrayList;
import java.util.Collections;
import java.util.Comparator;
import java.util.List;

public class CameraEnumerationAndroid {
    public static final ArrayList COMMON_RESOLUTIONS;
    public static final String TAG = "CameraEnumerationAndroid";

    public class CaptureFormat {
        public final FramerateRange framerate;
        public final int height;
        public final int imageFormat = 17;
        public final int width;

        public class FramerateRange {
            public int max;
            public int min;

            public boolean equals(Object obj) {
                if (!(obj instanceof FramerateRange)) {
                    return false;
                }
                FramerateRange framerateRange = (FramerateRange) obj;
                if (this.min == framerateRange.min && this.max == framerateRange.max) {
                    return true;
                }
                return false;
            }

            public int hashCode() {
                return (this.min * 65537) + 1 + this.max;
            }

            public String toString() {
                StringBuilder sb = new StringBuilder();
                sb.append("[");
                sb.append(((float) this.min) / 1000.0f);
                sb.append(":");
                sb.append(((float) this.max) / 1000.0f);
                return AnonymousClass7TF.A0l("]", sb);
            }

            public FramerateRange(int i, int i2) {
                this.min = i;
                this.max = i2;
            }
        }

        public static int frameSize(int i, int i2, int i3) {
            if (i3 == 17) {
                return ((i * i2) * ImageFormat.getBitsPerPixel(i3)) / 8;
            }
            throw new UnsupportedOperationException("Don't know how to calculate the frame size of non-NV21 image formats.");
        }

        public boolean equals(Object obj) {
            if (!(obj instanceof CaptureFormat)) {
                return false;
            }
            CaptureFormat captureFormat = (CaptureFormat) obj;
            if (this.width == captureFormat.width && this.height == captureFormat.height && this.framerate.equals(captureFormat.framerate)) {
                return true;
            }
            return false;
        }

        public int hashCode() {
            return AnonymousClass7TE.A0N(this.framerate, (((this.width * 65497) + this.height) * 251) + 1);
        }

        public String toString() {
            StringBuilder sb = new StringBuilder();
            sb.append(this.width);
            sb.append("x");
            sb.append(this.height);
            sb.append("@");
            return AnonymousClass7TF.A0i(this.framerate, sb);
        }

        public CaptureFormat(int i, int i2, FramerateRange framerateRange) {
            this.width = i;
            this.height = i2;
            this.framerate = framerateRange;
        }

        public CaptureFormat(int i, int i2, int i3, int i4) {
            this.width = i;
            this.height = i2;
            this.framerate = new FramerateRange(i3, i4);
        }

        public int frameSize() {
            return ((this.width * this.height) * ImageFormat.getBitsPerPixel(17)) / 8;
        }
    }

    public abstract class ClosestComparator implements Comparator {
        public abstract int diff(Object obj);

        public int compare(Object obj, Object obj2) {
            return diff(obj) - diff(obj2);
        }

        public /* synthetic */ ClosestComparator(AnonymousClass1 r1) {
        }

        public ClosestComparator() {
        }
    }

    static {
        Size size = new Size(160, 120);
        Size size2 = new Size(240, 160);
        Size size3 = new Size(320, 240);
        Size size4 = new Size(ScreenCapturerAndroid.VIRTUAL_DISPLAY_DPI, 240);
        Size size5 = new Size(480, 320);
        Size size6 = new Size(640, 360);
        Size size7 = new Size(640, 480);
        Size size8 = new Size(768, 480);
        Size size9 = new Size(854, 480);
        Size size10 = new Size(800, 600);
        Size size11 = new Size(960, 540);
        Size size12 = new Size(960, 640);
        Size size13 = size7;
        Size size14 = size8;
        Size size15 = size9;
        Size size16 = size10;
        Size size17 = size11;
        Size size18 = size12;
        Size size19 = size2;
        Size size20 = size3;
        Size size21 = size4;
        Size size22 = size5;
        Size size23 = size6;
        COMMON_RESOLUTIONS = Pxf.A0q(new Size[]{size, size19, size20, size21, size22, size23, size13, size14, size15, size16, size17, size18, new Size(1024, 576), new Size(1024, 600), new Size(1280, 720), new Size(1280, 1024), new Size(1920, 1080), new Size(1920, 1440), new Size(2560, 1440), new Size(3840, 2160)});
    }

    public static CaptureFormat.FramerateRange getClosestSupportedFramerateRange(List list, final int i) {
        return (CaptureFormat.FramerateRange) Collections.min(list, new ClosestComparator() {
            public static final int MAX_FPS_DIFF_THRESHOLD = 5000;
            public static final int MAX_FPS_HIGH_DIFF_WEIGHT = 3;
            public static final int MAX_FPS_LOW_DIFF_WEIGHT = 1;
            public static final int MIN_FPS_HIGH_VALUE_WEIGHT = 4;
            public static final int MIN_FPS_LOW_VALUE_WEIGHT = 1;
            public static final int MIN_FPS_THRESHOLD = 8000;

            private int progressivePenalty(int i, int i2, int i3, int i4) {
                int i5 = i * i3;
                if (i < i2) {
                    return i5;
                }
                return ((i - i2) * i4) + (i3 * i2);
            }

            public int diff(CaptureFormat.FramerateRange framerateRange) {
                int i = framerateRange.min;
                if (i >= 8000) {
                    i = ((i - MIN_FPS_THRESHOLD) * 4) + MIN_FPS_THRESHOLD;
                }
                int A06 = C13988Tno.A06(i * IgNetworkConsentStorage.MAX_ENTRIES, framerateRange.max);
                if (A06 >= 5000) {
                    A06 = ((A06 - 5000) * 3) + 5000;
                }
                return i + A06;
            }
        });
    }

    public static Size getClosestSupportedSize(List list, final int i, final int i2) {
        return (Size) Collections.min(list, new ClosestComparator() {
            public int diff(Size size) {
                return C13988Tno.A06(i, size.width) + C13988Tno.A06(i2, size.height);
            }
        });
    }

    public static void reportCameraResolution(Histogram histogram, Size size) {
        histogram.addSample(COMMON_RESOLUTIONS.indexOf(size) + 1);
    }
}
