...
public class TreeJNI extends HybridClassBase {
    ...
    public final JSONObject asJSON(boolean z) {
        ...
        for (String str2 : getScalarFields()) {
            if (isFieldSet(str2)) {
                ...
                if (field_UNTYPED == null) {
                    ...
                    for (Object put : (List) field_UNTYPED) {
                        jSONArray2.put(put);
                    }
        ...
        for (Edge edge : getEdgeFields()) {
            if (isFieldSet(edge.field)) {
                if (edge.isPlural) {
                    ...
                    C249803kO it = getTreeList(edge.field, edge.treeClass).iterator();
                    while (it.hasNext()) {
                        jSONArray3.put(((TreeJNI) it.next()).asJSON(z));
                    }
                    str = edge.field;
        ...
        for (Class reinterpret : getInlineClasses()) {
            JSONObject asJSON = reinterpret(reinterpret).asJSON(z);
            if (z) {
    ...
    public final ImmutableList getEnumList(String str, Enum enumR) {
        ...
        C249803kO it = stringList.iterator();
        while (it.hasNext()) {
            builder.add(parseEnum((String) it.next(), enumR));
        }
        return builder.build();
    }

    public final Enum getEnumValue(String str, Enum enumR) {
        return parseEnum(getStringValue(str), enumR);
    }
